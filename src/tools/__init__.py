...
def get_travel_tools():
    ...
    def hotel_budget_tool_func(input_str: str) -> str:
        try:
            ...
            return f"参数格式错误，请使用格式：城市,预算级别（如：北京,经济型）。错误：{e}"
    ...
    def driving_route_tool_func(input_str: str) -> str:
        try:
            ...
            data = RouteTool.get_driving_route(origin.strip(), destination.strip())
            if data:
                return RouteTool.format_driving_route(data)
            return "抱歉，暂时无法获取路线信息"
        except Exception as e:
            return f"参数格式错误，请使用格式：起点|终点。错误：{e}"

    tools.append(Tool(
        name="驾车路线规划",
        func=driving_route_tool_func,
        description="查询两点之间的驾车路线，输入格式：起点|终点（如：天安门|故宫博物院）",
    ))
    ...
    def transit_route_tool_func(input_str: str) -> str:
        try:
            ...
            if len(parts) == 3:
                origin, destination, city = parts[0].strip(), parts[1].strip(), parts[2].strip()
            else:
    ...
    tools.append(Tool(
        ...
        description="查询两点之间的公交/地铁路线，输入格式：起点|终点|所在城市",
    ))
    ...
    def multi_spot_route_tool_func(input_str: str) -> str:
        try:
            ...
            if len(parts) < 2:
                return "至少需要2个景点才能规划路线，格式：城市|景点1|景点2|景点3|..."
            city = parts[0]
            spots = parts[1:]
            ...
            return f"参数格式错误，请使用格式：城市|景点1|景点2|景点3。错误：{e}"
